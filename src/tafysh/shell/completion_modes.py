...
Provides configurable completion modes that integrate TafySH completions
...
def merge_completions(
    tafysh_completions: list[str],
    shell_completions: list[str],
    config: Optional[CompletionConfig] = None,
) -> list[str]:
    """Merge TafySH and shell completions with deduplication.

    Args:
        tafysh_completions: Completions from TafySH
    ...
    # TafySH completions have higher priority
    for i, comp in enumerate(tafysh_completions):
        if comp not in seen:
            seen[comp] = i

    # Add shell completions that aren't duplicates
    offset = len(tafysh_completions)
    for i, comp in enumerate(shell_completions):
        if comp not in seen:
            seen[comp] = offset + i

    # Sort by original order (TafySH first, then shell)
