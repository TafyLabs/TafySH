"""Login shell support for TafySH.

Provides functionality for running TafySH as a login shell:
...
class SessionInfo:
    ...
    def create(cls, is_login: bool = False) -> "SessionInfo":
        ...
        return cls(
            ...
            home=Path.home(),
            shell=sys.argv[0] if sys.argv else "tafysh",
            pid=os.getpid(),
        )
...
class LoginShellManager:
    ...
    def is_login_shell(self) -> bool:
        ...
        - argv[0] starting with '-' (e.g., '-tafysh')
        - --login or -l flag
    ...
    def setup_environment(self) -> None:
        ...
        # Set session ID
        os.environ["TAFYSH_SESSION_ID"] = session.session_id
        ...
        # Set shell name
        os.environ["TAFYSH_SHELL"] = "tafysh"
        ...
        # Set SHELL to tafysh path if we're a login shell
        if session.is_login_shell:
            import shutil
            tafysh_path = shutil.which("tafysh")
            if tafysh_path:
                os.environ["SHELL"] = tafysh_path
    ...
    def source_rc_files(self, shell: str = "bash") -> None:
        ...
        # Source TafySH-specific RC
        if self._custom_rc:
            self._source_file(self._custom_rc, shell)
        else:
            tafysh_rc = home / self.TAFYSH_RC
            self._source_tafysh_rc(tafysh_rc)
    ...
    def _source_tafysh_rc(self, path: Path) -> bool:
        """Source TafySH RC file.

        The .tafyshrc file can contain:
        - Shell aliases (passed to underlying shell)
        - TafySH configuration commands
        - Tool registrations
        - Custom prompts

        Args:
            path: Path to .tafyshrc
        ...
        try:
            ...
            # Parse and execute TafySH-specific commands
            # For now, just track that we would source it
            self._sourced_files.append(str(path))
            logger.debug("Sourced tafyshrc", path=str(path))
            return True
        except Exception as e:
            logger.debug("Failed to source tafyshrc", path=str(path), error=str(e))
            return False
