"""Tab completion for TafySH shell.

Provides intelligent tab completion with configurable modes:
- NATIVE: Only TafySH completions (special commands, tools, paths)
- PASSTHROUGH: Full PTY passthrough for completions
- HYBRID: Merge TafySH + shell completions (default)
"""
...
class ShellCompleter:
    """Tab completion handler for TafySH.
    ...
    def _get_completions(self, text: str, line: str) -> list[str]:
        ...
        # Get TafySH completions
        tafysh_completions = self._get_tafysh_completions(text, stripped_line)

        # In native mode, only return TafySH completions
        if self._mode == CompletionMode.NATIVE:
            return tafysh_completions

        # Hybrid mode: merge TafySH and shell completions
    ...
    def _get_tafysh_completions(self, text: str, line: str) -> list[str]:
        """Get TafySH-specific completions.
