...
from tafysh.config.loader import load_config
from tafysh.telemetry.logger import setup_logging, get_logger
...
For more information, visit: https://github.com/TafyLabs/TafySH
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="FILE",
        help="Use FILE instead of ~/.tafyshrc",
    )
...
def cmd_config_show(config_path: Optional[Path]) -> int:
    ...
    try:
        ...
        print("Current TafySH Configuration:")
        print("=" * 50)
...
def cmd_config_init(config_path: Optional[Path]) -> int:
    ...
    from tafysh.config.loader import get_default_config_path, create_default_config
...
def cmd_status() -> int:
    """Check system health."""
    from tafysh.telemetry.health import HealthChecker

    print("TafySH System Status")
    print("=" * 50)
...
def cmd_mcp_server(config_path: Optional[Path]) -> int:
    ...
    from tafysh.orchestrator.mcp_server import MCPServer, MCPServerConfig
...
def cmd_devices(args: argparse.Namespace) -> int:
    ...
    inventory_path = Path("~/.tafysh/devices.yaml").expanduser()
    ...
    if args.devices_command == "list":
        ...
        if device_id:
            ...
            from tafysh.orchestrator.ssh import SSHCredentials, SSHExecutor
...
def cmd_interactive_shell(
    ...
    from tafysh.shell.login import LoginShellManager
    from tafysh.shell.session import get_session, cleanup_session
    ...
    try:
        ...
        logger.info("Starting TafySH", version=__version__)
        ...
        print(f"Error starting TafySH: {e}", file=sys.stderr)
        return 1
