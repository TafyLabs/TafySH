...
def get_default_config_path() -> Path:
    ...
    return Path.home() / ".tafysh" / "config.yaml"
...
def get_config_paths() -> list[Path]:
    ...
    # System-wide config
    system_config = Path("/etc/tafysh/config.yaml")
    ...
    project_config = Path.cwd() / ".tafysh.yaml"
...
def get_env_overrides() -> dict[str, Any]:
    ...
    Environment variables are prefixed with TAFYSH_ and use double underscores
    for nested keys. For example:
    - TAFYSH_LOG_LEVEL=DEBUG -> {"log_level": "DEBUG"}
    - TAFYSH_LLM__PROVIDER=openai -> {"llm": {"provider": "openai"}}
    """
...
def load_config(
    ...
    2. System config (/etc/tafysh/config.yaml)
    3. User config (~/.tafysh/config.yaml)
    4. Project config (.tafysh.yaml in cwd)
    5. Explicit config file (--config argument)
    6. Environment variables (TAFYSH_*)
    ...
    Returns:
        Validated TafySHConfig instance
    """
    ...
    return TafySHConfig(**merged_config)
...
def create_default_config(path: Path) -> None:
    ...
    default_config = TafySHConfig()

    # Convert to dict and then to YAML
    config_dict = default_config.model_dump(exclude_none=True)

    # Add helpful comments
    yaml_content = """# TafySH Configuration
# See documentation for all options: https://github.com/TafyLabs/TafySH/docs
