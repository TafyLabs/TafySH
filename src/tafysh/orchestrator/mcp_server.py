"""Model Context Protocol (MCP) server for TafySH.

Provides an MCP-compatible server that exposes TafySH tools
...
from tafysh.telemetry.health import check_health
...
class MCPServer:
    ...
    Exposes TafySH tools via the Model Context Protocol for
                ...
                "resources": [
                    {
                        "uri": "tafysh://health",
                        "name": "Health Status",
                        "description": "Current health status of TafySH",
...
def main() -> None:
    ...
    config = MCPServerConfig(
        name=os.environ.get("TAFYSH_MCP_NAME", "tafysh"),
        version=os.environ.get("TAFYSH_MCP_VERSION", "1.0.0"),
        require_auth=os.environ.get("TAFYSH_MCP_AUTH", "").lower() == "true",
        auth_token=os.environ.get("TAFYSH_MCP_TOKEN"),
    )

    # Parse allowed tools from environment
    allowed_tools_env = os.environ.get("TAFYSH_MCP_TOOLS")
