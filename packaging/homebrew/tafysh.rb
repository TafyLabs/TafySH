...
class Tafysh < Formula
  ...
  homepage "https://github.com/tafysh/tafysh"
  url "https://files.pythonhosted.org/packages/source/a/tafysh/tafysh-0.1.0.tar.gz"
  sha256 "PLACEHOLDER_SHA256"
  license "MIT"
  head "https://github.com/tafysh/tafysh.git", branch: "main"
  ...
  def install
    ...
    generate_completions_from_executable(bin/"tafysh", "--completions", shells: [:bash, :zsh, :fish])
  end

  def post_install
    # Add to /etc/shells if not present
    shells_file = "/etc/shells"
    tafysh_path = "#{HOMEBREW_PREFIX}/bin/tafysh"

    if File.exist?(shells_file)
      shells_content = File.read(shells_file)
      unless shells_content.include?(tafysh_path)
        ohai "To use TafySH as a login shell, add it to /etc/shells:"
        ohai "  echo '#{tafysh_path}' | sudo tee -a /etc/shells"
      end
    end
  end
  ...
  def caveats
    <<~EOS
      TafySH has been installed!

      To start using TafySH:
        tafysh

      To configure your API keys:
        tafysh config init

      To set TafySH as your default shell:
        1. Add to /etc/shells:
           echo '#{HOMEBREW_PREFIX}/bin/tafysh' | sudo tee -a /etc/shells
        2. Change your shell:
           chsh -s #{HOMEBREW_PREFIX}/bin/tafysh

      For more information:
        https://github.com/tafysh/tafysh
    EOS
  end

  test do
    assert_match "tafysh", shell_output("#{bin}/tafysh --version")

    # Test help output
    assert_match "AI-enhanced", shell_output("#{bin}/tafysh --help")

    # Test that it starts (with immediate exit)
    system bin/"tafysh", "--help"
  end
end
